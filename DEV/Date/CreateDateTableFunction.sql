CREATE OR ALTER FUNCTION udf_GenerateDateDimensionColumns (@Date date)
RETURNS TABLE
AS
RETURN 
...
SELECT @Date                                             AS [Date]                             -- 2013-01-01
	 ...
	 , (SELECT (DATEPART(weekday, @Date) + @@DATEFIRST - 2) % 7 + 1) AS [Day of Week Number]   -- 3
...
,N'FY' + CAST(CASE WHEN MONTH(@Date) > 3
                    THEN YEAR(@Date) + 1
                    ELSE YEAR(@Date)
                END AS NVARCHAR(4))
+N'-Q'
+ CASE WHEN MONTH(@Date) > 3
        THEN CAST(DATEPART(quarter, @Date) - 1 
                AS NVARCHAR(2)
                )
        ELSE CAST(DATEPART(quarter, @Date) + 3
                AS NVARCHAR(2)
                )
  END                                            AS [Fiscal Quarter UK Label]             
, CASE WHEN MONTH(@Date) > 3 THEN 1 ELSE 2 END      AS [Fiscal Half of Year UK Number]      
, N'FY' + CAST(CASE WHEN MONTH(@Date) > 3
                    THEN YEAR(@Date) + 1
                    ELSE YEAR(@Date)
                END AS NVARCHAR(4))
+ N'-H' 
+ CASE WHEN MONTH(@Date) > 3  and MONTH(@Date)<10
        THEN N'1' 
        WHEN Year(@Date)*100+MONTH(@Date)>=(Year(@Date)-1)*100+10 or (Year(@Date))*100+MONTH(@Date)<=3 Then N'2' 
		
    END                                      AS [Fiscal Half of Year UK Label]        
, CASE WHEN MONTH(@Date) > 3
    THEN YEAR(@Date) + 1
    ELSE YEAR(@Date)
END                                              AS [Fiscal Year UK]                      
, N'FY' + CAST(CASE WHEN MONTH(@Date) > 3                                                 
                    THEN YEAR(@Date) + 1                                                  
                    ELSE YEAR(@Date)                                                      
                END AS NVARCHAR(4))                 AS [Fiscal Year UK Label]
GO

SELECT * FROM udf_GenerateDateDimensionColumns (GETDATE() - 100)
GO
